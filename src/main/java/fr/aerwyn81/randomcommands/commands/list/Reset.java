...
public class Reset implements Cmd {
    ...
    public boolean perform(CommandSender sender, String[] args) {
        ...
        var id = args[1];
        if (runningCommands.get(id).isEmpty()) {
    ...
    public ArrayList<String> tabComplete(CommandSender sender, String[] args) {
        return args.length == 2 ? new ArrayList<>(StreamSupport.stream(RandomCommands.runningCommands.spliterator(), false).filter(s -> s.startsWith(args[1])).collect(Collectors.toList())) : new ArrayList<>();
    }
}
