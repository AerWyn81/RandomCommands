...
public class LanguageService {
	...
	public static void initialize(String lang) {
		new File(RandomCommands.getInstance().getDataFolder() + "/language").mkdirs();
	...
	public static String checkLanguage(String lang) {
		File f = new File(RandomCommands.getInstance().getDataFolder() + "/language/messages_" + lang + ".yml");
		if (f.exists())
			return lang;
		return "en";
	}
	...
	public static void pushMessages() {
		File f = new File(RandomCommands.getInstance().getDataFolder() + "/language/messages_" + language + ".yml");
	...
	public static void loadLanguage(String lang) {
		File file = new File(RandomCommands.getInstance().getDataFolder() + "/language/messages_" + lang + ".yml");
		if (!file.exists()) {
			try {
				file.createNewFile();
			} catch (IOException e) {
		...
		InputStreamReader input = new InputStreamReader(RandomCommands.getInstance().getResource("language/messages_" + lang + ".yml"), StandardCharsets.UTF_8);
		...
		try {
			ConfigUpdater.update(RandomCommands.getInstance(), "language/messages_" + lang + ".yml", new File(RandomCommands.getInstance().getDataFolder() + "/language/messages_" + lang + ".yml"), Collections.emptyList());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
